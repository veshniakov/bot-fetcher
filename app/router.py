...
def create_router(
    ...
    async def link_message(message: Message, bot: Bot) -> None:
        ...
        if not url:
            if message.text == "📊 Статистика":
                # Обрабатывается в отдельном хэндлере, пропускаем
                return
    ...
    async def confirm_download_callback(callback: CallbackQuery, bot: Bot) -> None:
        ...
        if download_semaphore.locked():
            if callback.message:
                await callback.message.answer("Очередь на скачивание занята. Ожидайте...")
...
async def _show_stats(message: Message, stats: StatsManager, store: PendingTaskStore, settings: Settings) -> None:
    ...
    text = (
        ...
        f"📥 Всего скачано видео: {downloads}\n\n"
        "⚙️ <b>Система:</b>\n"
        f"🔄 В работе (качается): {active_tasks} из 10\n"
        f"💾 Диск: свободно {free_gb:.1f} GB"
    )
